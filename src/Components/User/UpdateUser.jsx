...
const UpdateUser = ({ userId, onUserUpdated, formClasses, inputClasses, buttonClasses, handleCancel }) => {
  ...
  useEffect(() => {
    fetchUserById(userId)
      .then((data) => {
        setUser(data);
        setLoading(false);
        usernameRef.current?.focus(); // Autofocus on username field when loaded
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation: Username must contain at least one alphabetical character
    const usernamePattern = /.*[a-zA-Z].*/;
    if (!usernamePattern.test(user.username)) {
      setErrorMessage("Username must contain at least one letter.");
      return;
    } else {
      setErrorMessage("");
    }
  ...
  const handleEmailInvalid = (e) => {
    if (!e.target.validity.valid) {
      setEmailErrorMessage("Invalid email format. Example: user@example.com");
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className={formClasses}>
      ...
      <div className="mt-6 flex justify-between">
        <button type="submit" className={`${buttonClasses} mr-2`}>
          Update User
        </button>
        <button
          type="button"
